...
class DeepSort(object):
    def __init__(self, model_path, max_dist=0.2, min_confidence=0.3, nms_max_overlap=1.0, max_iou_distance=0.7,
                 max_age=70, n_init=3, nn_budget=100, feature_dim=128, use_cuda=True):
        self.min_confidence = min_confidence
        self.nms_max_overlap = nms_max_overlap

        self.extractor = Extractor(model_path, use_cuda=use_cuda, feature_dim=feature_dim)
    ...
    def update(self, bboxes, ori_img, cls=None):
        ...
        for track in self.tracker.tracks:
            temp_id.append(track.track_id)   # 返回所有非delete的id，包括is_confirmed的id，用来去除kitchen中的misBehavior
            if not track.is_confirmed() or track.time_since_update > 1:
                continue
            box = track.to_tlwh()
            x1, y1, x2, y2 = self._tlwh_to_xyxy(box)
            track_id = track.track_id
            if track.cls is not None:
                outputs.append(np.array([x1, y1, x2, y2, track_id, track.cls], dtype=np.int))
            else:
                outputs.append(np.array([x1, y1, x2, y2, track_id], dtype=np.int))
        if len(outputs) > 0:
            outputs = np.stack(outputs, axis=0)
        return outputs, temp_id
    ...
    def _get_features(self, bbox_xywh, ori_img, xyxy=True):
        im_crops = []
        for box in bbox_xywh:
            if xyxy:
                x1, y1, x2, y2 = [int(box[bi]) for bi in range(4)]
            else:
